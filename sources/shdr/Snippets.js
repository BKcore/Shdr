...
(function() {
  ...
  Snippets = {
    ...
    'Blin-Phong (Dir)': ['vec2 blinnPhongDir(vec3 lightDir, float lightInt, float Ka, float Kd, float Ks, float shininess)', '{', '  vec3 s = normalize(lightDir);', '  vec3 v = normalize(-fPosition);', '  vec3 n = normalize(fNormal);', '  vec3 h = normalize(v+s);', '  float diffuse = Ka + Kd * lightInt * max(0.0, dot(n, v));', '  float spec =  Ks * pow(max(0.0, dot(n,h)), shininess);', '  return vec2(diffuse, spec);', '}'].join('\n'),
